"""Extraction history page showing OCR/LLM text extraction results.

A simplified version of the translation HistoryPage — no pause/resume,
no progress tracking, just a table of completed/failed extractions with
Open, Re-extract, and Delete actions.
"""
...
from src.constants.history import STATUS_EXTRACTING, STATUS_PENDING, display_status
...
class ExtractionHistoryPage(QWidget):
    """History page displaying OCR extraction results."""

    # Emitted when user requests re-extract: (tasks,)
    re_extract_requested = Signal(list)
    ...
    def _setup_ui(self) -> None:
        ...
        self.page, self.layout = create_page_container(
            tr("page.extraction_history"),
            tr_key="page.extraction_history",
        )
        ...
        self.search_input.setPlaceholderText(
            tr("extraction_history.search_placeholder")
        )
        ...
        self.re_extract_btn = self._create_action_button(
            tr("btn.re_extract"), style_primary_button(), self.on_re_extract
        )
    ...
    def _setup_table(self) -> None:
        """Configures the extraction history table widget."""
    ...
    def apply_theme(self) -> None:
        ...
        self.re_extract_btn.setStyleSheet(style_primary_button())
    ...
    def apply_language(self) -> None:
        ...
        self.re_extract_btn.setText(tr("btn.re_extract"))
    ...
    def refresh_history(self, force: bool = False) -> None:
        ...
        fingerprint = get_extraction_fingerprint()
        if (
            not force
    ...
    def _fill_row(
        ...
        elif status_lower == STATUS_EXTRACTING.lower():
    ...
    def _update_button_states(self) -> None:
        ...
        for row in selected_rows:
            ...
            if raw_status in (STATUS_PENDING, STATUS_EXTRACTING):
                is_active_selected = True

        self.open_btn.setEnabled(has_selection)
        self.re_extract_btn.setEnabled(has_selection and not is_active_selected)
    ...
    def on_re_extract(self) -> None:
        """Re-extracts selected images with a new source language."""
        selected_rows = sorted({item.row() for item in self.table.selectedItems()})
        ...
        self.re_extract_requested.emit(tasks)
    ...
    def on_delete_selected(self) -> None:
        """Removes selected extraction history entries and output files."""
        ...
        for row in selected_rows:
            entry_id = self.table.item(row, 0).data(Qt.ItemDataRole.UserRole)
            output_path = delete_extraction_entry(entry_id)
            if output_path:
                p = Path(output_path)
                if p.is_file():
