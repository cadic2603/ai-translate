"""Voice history page showing text-to-speech generation results.

A simplified version of SubtitleHistoryPage — shows completed/failed
voice generations with Open, Re-generate, and Delete actions.
"""
...
class VoiceHistoryPage(QWidget):
    """History page displaying voice generation results."""
    ...
    def _setup_ui(self) -> None:
        ...
        self.page, self.layout = create_page_container(
            tr("page.voice_history"),
            tr_key="page.voice_history",
        )
        ...
        self.search_input.setPlaceholderText(tr("voice_history.search_placeholder"))
    ...
    def _setup_table(self) -> None:
        """Configures the voice history table widget."""
    ...
    def refresh_history(self, force: bool = False) -> None:
        ...
        fingerprint = get_voice_fingerprint()
        if (
            not force
    ...
    def on_delete_selected(self) -> None:
        """Removes selected voice history entries and output files."""
        selected_rows = sorted(
            {item.row() for item in self.table.selectedItems()},
            reverse=True,
        )
        ...
        for row in selected_rows:
            entry_id = self.table.item(row, 0).data(Qt.ItemDataRole.UserRole)
            output_path = delete_voice_entry(entry_id)
            if output_path:
                p = Path(output_path)
                if p.is_file():
