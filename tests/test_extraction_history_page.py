"""Comprehensive tests for ExtractionHistoryPage.

Covers page creation, widget structure, refresh_history, _fill_row,
_on_header_clicked, on_open_file, on_delete_selected, on_re_extract,
...
# ---------------------------------------------------------------------------
_MOD = "src.ui.pages.extraction_history"
...
def _mock_db():
    ...
    with (
        patch(f"{_MOD}.get_extraction_fingerprint", return_value=None),
        patch(f"{_MOD}.get_extraction_history", return_value=[]),
    ):
        yield
...
def page(_mock_db, qtbot):
    """Creates an ExtractionHistoryPage widget for testing."""
    from src.ui.pages.extraction_history import ExtractionHistoryPage  # noqa: PLC0415

    p = ExtractionHistoryPage()
    qtbot.addWidget(p)
    return p
...
def _make_entry(  # noqa: PLR0913
    ...
    name: str = "image.png",
    file_size: int = 1024,
    source_path: str = "/tmp/source/image.png",
    output_path: str = "/tmp/output/image.txt",
    status: str = "Done",
    error_message: str | None = None,
    created_at: str = "2026-01-15 10:30:00",
) -> tuple:
    """Helper to build an extraction history DB tuple."""
    return (
        entry_id,
        name,
        file_size,
...
def _populate_table(page, entries):  # noqa: ANN001, ANN202
    ...
    with (
        patch(
            f"{_MOD}.get_extraction_fingerprint",
            return_value=(len(entries), len(entries), "x"),
        ),
        patch(f"{_MOD}.get_extraction_history", return_value=entries),
    ):
...
class TestConstruction:
    """Tests for ExtractionHistoryPage widget construction."""
    ...
    def test_has_action_buttons(self, page) -> None:  # noqa: ANN001
        """Page has open, re-extract, and delete buttons."""
...
class TestRefreshHistory:
    ...
    def test_refresh_populates_table(self, page) -> None:  # noqa: ANN001
        ...
        entries = [_make_entry(1), _make_entry(2, name="photo.jpg")]
    ...
    def test_refresh_clears_on_none(self, page) -> None:  # noqa: ANN001
        """Table is cleared when get_extraction_history returns None."""
        with (
            patch(f"{_MOD}.get_extraction_fingerprint", return_value=(0, 0, "x")),
            patch(f"{_MOD}.get_extraction_history", return_value=None),
        ):
            page.refresh_history(force=True)
    ...
    def test_refresh_skips_when_fingerprint_unchanged(self, page) -> None:  # noqa: ANN001
        ...
        # Make the widget visible first (showEvent triggers a refresh)
        with (
            patch(f"{_MOD}.get_extraction_fingerprint", return_value=fp),
            patch(f"{_MOD}.get_extraction_history", return_value=[]),
        ):
            page.show()
        # Now test the skip logic with a fresh mock
        with (
            patch(f"{_MOD}.get_extraction_fingerprint", return_value=fp),
            patch(f"{_MOD}.get_extraction_history", history_mock),
        ):
...
class TestFillRow:
    ...
    def test_stores_output_path(self, page) -> None:  # noqa: ANN001
        ...
        _populate_table(page, [_make_entry(output_path="/out/file.txt")])
        item = page.table.item(0, 0)
        assert item.data(Qt.ItemDataRole.UserRole + 1) == "/out/file.txt"

    def test_stores_source_path(self, page) -> None:  # noqa: ANN001
        """Source path is stored in UserRole+2 on column 0."""
        _populate_table(page, [_make_entry(source_path="/src/img.png")])
        item = page.table.item(0, 0)
        assert item.data(Qt.ItemDataRole.UserRole + 2) == "/src/img.png"

    def test_stores_error_message(self, page) -> None:  # noqa: ANN001
        """Error message is stored in UserRole+3 on column 0."""
        _populate_table(page, [_make_entry(error_message="ERR_OCR")])
        item = page.table.item(0, 0)
        assert item.data(Qt.ItemDataRole.UserRole + 3) == "ERR_OCR"

    def test_file_name_displayed(self, page) -> None:  # noqa: ANN001
        """File name is shown as column 0 text."""
        _populate_table(page, [_make_entry(name="screenshot.png")])
        assert page.table.item(0, 0).text() == "screenshot.png"

    def test_size_formatted(self, page) -> None:  # noqa: ANN001
        """File size is formatted via format_file_size."""
        _populate_table(page, [_make_entry(file_size=1024)])
    ...
    def test_status_done_color(self, page) -> None:  # noqa: ANN001
        """Done status is displayed with success color."""
    ...
    def test_status_failed_color(self, page) -> None:  # noqa: ANN001
        """Failed status is displayed with error color."""
        _populate_table(page, [_make_entry(status="Failed")])
        status_item = page.table.item(0, 2)
        assert status_item.data(Qt.ItemDataRole.UserRole) == "Failed"

    def test_status_extracting_stored(self, page) -> None:  # noqa: ANN001
        """Extracting status raw value is stored in UserRole."""
        _populate_table(page, [_make_entry(status="Extracting")])
        status_item = page.table.item(0, 2)
        assert status_item.data(Qt.ItemDataRole.UserRole) == "Extracting"
...
class TestButtonStates:
    ...
    def test_done_selection_enables_all(self, page) -> None:  # noqa: ANN001
        ...
        assert page.re_extract_btn.isEnabled()
        assert page.delete_btn.isEnabled()

    def test_pending_disables_re_extract(self, page) -> None:  # noqa: ANN001
        """Selecting a Pending entry disables re-extract but enables open/delete."""
        _populate_table(page, [_make_entry(status="Pending")])
        page.table.selectRow(0)
        assert page.open_btn.isEnabled()
        assert not page.re_extract_btn.isEnabled()
    ...
    def test_extracting_disables_re_extract(self, page) -> None:  # noqa: ANN001
        """Selecting an Extracting entry disables re-extract."""
        _populate_table(page, [_make_entry(status="Extracting")])
        page.table.selectRow(0)
        assert not page.re_extract_btn.isEnabled()
    ...
    def test_error_message_shown_for_failed(self, page) -> None:  # noqa: ANN001
        ...
        page.show()
        _populate_table(page, [_make_entry(status="Failed", error_message="ERR_TEST")])
        page.table.selectRow(0)
    ...
    def test_auth_error_service_suffix_renders_specific_service(
        ...
        Extract Text routes to OCR (Google Cloud) OR LLM vision
        (Gemini / Vertex AI / Custom).  Pins the service-aware
        copy so the user can target the right Settings tab.
        """
        ...
        _populate_table(
            page,
            [_make_entry(status="Failed", error_message="AUTH_ERROR:Google Cloud")],
        )
        page.table.selectRow(0)
...
class TestOpenFile:
    ...
    def test_open_file_skips_missing(
        ...
        _populate_table(page, [_make_entry(output_path="/out/missing.txt")])
        page.table.selectRow(0)
        page.on_open_file()
...
class TestDeleteSelected:
    ...
    @patch(f"{_MOD}.delete_extraction_entry", return_value="/out/file.txt")
...
class TestReExtract:
    """Tests for on_re_extract behavior."""

    @patch(f"{_MOD}.Path.exists", return_value=True)
    def test_re_extract_emits_signal(
        self,
        mock_exists,
        page,
        qtbot,  # noqa: ANN001
    ) -> None:
        """Re-extract emits re_extract_requested with correct tasks."""
        _populate_table(
            page,
            [_make_entry(entry_id=7, source_path="/src/img.png", status="Done")],
        )
        page.table.selectRow(0)

        with qtbot.waitSignal(page.re_extract_requested, timeout=1000) as sig:
            page.on_re_extract()

        assert len(sig.args[0]) == 1
        assert sig.args[0][0] == (7, "/src/img.png")

    @patch(f"{_MOD}.Path.exists", return_value=False)
    @patch(f"{_MOD}.CustomMessageDialog.show_message")
    @patch(f"{_MOD}.delete_extraction_entry")
    def test_re_extract_missing_source(
        self,
        mock_delete,
        mock_msg,
        mock_exists,
        ...
        _populate_table(page, [_make_entry(entry_id=9, source_path="/gone.png")])
        page.table.selectRow(0)
    ...
    def test_re_extract_no_selection(self, page) -> None:  # noqa: ANN001
        """Does nothing when no rows are selected."""
        _populate_table(page, [_make_entry()])
        page.table.clearSelection()
        page.on_re_extract()  # Should not raise
...
class TestSearchFiltering:
    ...
    def test_search_filters_by_name(self, page) -> None:  # noqa: ANN001
        ...
        entries = [
            _make_entry(entry_id=1, name="Screenshot.png"),
            _make_entry(entry_id=2, name="Photo.jpg"),
        ]
        page.search_input.setText("screen")
        _populate_table(page, entries)
    ...
    def test_search_no_match_shows_none(self, page) -> None:  # noqa: ANN001
        ...
        entries = [_make_entry(entry_id=1, name="image.png")]
...
class TestSelectionPreservation:
    ...
    def test_selection_restored_after_refresh(self, page) -> None:  # noqa: ANN001
        ...
        # Re-populate with same data — selection should be restored
        _populate_table(page, entries)
...
class TestThemeAndLanguage:
    ...
    def test_apply_language_updates_button_text(self, page) -> None:  # noqa: ANN001
        ...
        assert page.re_extract_btn.text()
...
class TestSortItems:
    ...
    def test_numerical_sort_item_ordering(self, page) -> None:  # noqa: ANN001
        ...
        # Lexicographic would say "10" < "9", but numerical must say 9 < 10
        assert nine < ten
        assert ten < hundred
        assert not ten < nine
...
class TestPageIsQWidget:
    ...
    def test_is_qwidget_instance(self, page) -> None:  # noqa: ANN001
        """ExtractionHistoryPage is a QWidget."""
    ...
    def test_buttons_have_cursor(self, page) -> None:  # noqa: ANN001
        ...
        for btn in [page.open_btn, page.re_extract_btn, page.delete_btn]:
...
class TestTableRefreshPreservation:
    ...
    def test_refresh_not_visible_no_force_skips(self, page) -> None:  # noqa: ANN001
        ...
        with (
            patch(f"{_MOD}.get_extraction_fingerprint", return_value=(1, 1, "new")),
            patch(f"{_MOD}.get_extraction_history", history_mock),
        ):
            page.refresh_history(force=False)
            history_mock.assert_not_called()
...
class TestSearchEdgeCases:
    ...
    def test_search_updates_highlight_delegate(self, page) -> None:  # noqa: ANN001
        ...
        entries = [_make_entry(entry_id=1, name="test.png")]
...
class TestUnicodeAndLongFilenames:
    ...
    def test_unicode_filename(self, page) -> None:  # noqa: ANN001
        ...
        _populate_table(page, [_make_entry(name="\u56fe\u7247\u6587\u4ef6.png")])
        assert page.table.item(0, 0).text() == "\u56fe\u7247\u6587\u4ef6.png"

    def test_emoji_filename(self, page) -> None:  # noqa: ANN001
        """Emoji in filenames are displayed correctly."""
        _populate_table(page, [_make_entry(name="\U0001f4f7 photo.png")])
        assert page.table.item(0, 0).text() == "\U0001f4f7 photo.png"
    ...
    def test_unicode_search(self, page) -> None:  # noqa: ANN001
        ...
        entries = [
            _make_entry(entry_id=1, name="\u56fe\u7247.png"),
            _make_entry(entry_id=2, name="english.png"),
        ]
        page.search_input.setText("\u56fe\u7247")
        _populate_table(page, entries)
        assert page.table.rowCount() == 1
...
class TestStatusColors:
    ...
    def test_extracting_status_has_primary_color(self, page) -> None:  # noqa: ANN001
        """Extracting status foreground uses primary color."""
        from PySide6.QtGui import QColor  # noqa: PLC0415
...
# ===================================================================
# Re-Extract Multiple Files
# ===================================================================


class TestReExtractMultiple:
    """Tests for re-extracting multiple files."""

    @patch(f"{_MOD}.Path.exists", return_value=True)
    def test_re_extract_multiple_emits_all(
        self,
        mock_exists,
        page,
        qtbot,  # noqa: ANN001
    ) -> None:
        """Re-extract with multiple selected emits all tasks."""
        entries = [
            _make_entry(entry_id=1, source_path="/src/i1.png", status="Done"),
            _make_entry(entry_id=2, source_path="/src/i2.png", status="Done"),
        ]
        _populate_table(page, entries)
        page.table.selectAll()
...
class TestDeleteFileCleanupExtended:
    ...
    @patch(f"{_MOD}.delete_extraction_entry", return_value=None)
    @patch(f"{_MOD}.CustomConfirmDialog.confirm", return_value=True)
...
class TestSortingBehavior:
    ...
    def test_sort_by_name_column(self, page) -> None:  # noqa: ANN001
        ...
        entries = [
            _make_entry(entry_id=1, name="Zebra.png"),
            _make_entry(entry_id=2, name="Apple.png"),
        ]
        _populate_table(page, entries)
        page.table.sortByColumn(0, Qt.SortOrder.AscendingOrder)
        assert page.table.item(0, 0).text() == "Apple.png"
        assert page.table.item(1, 0).text() == "Zebra.png"
...
class TestSignals:
    ...
    def test_re_extract_requested_signal_exists(self, page) -> None:  # noqa: ANN001
        """ExtractionHistoryPage has re_extract_requested signal."""
        assert hasattr(page, "re_extract_requested")
...
class TestFailedSelectionStates:
    ...
    def test_failed_enables_re_extract(self, page) -> None:  # noqa: ANN001
        """Selecting a Failed entry enables re-extract."""
        _populate_table(page, [_make_entry(status="Failed")])
        page.table.selectRow(0)
        assert page.re_extract_btn.isEnabled()

    def test_mixed_done_and_pending_disables_re_extract(
        self,
        page,  # noqa: ANN001
    ) -> None:
        """Mixed Done + Pending selection disables re-extract."""
        entries = [
    ...
    def test_mixed_done_and_extracting_disables_re_extract(
        self,
        page,  # noqa: ANN001
    ) -> None:
        """Mixed Done + Extracting selection disables re-extract."""
        entries = [
            _make_entry(entry_id=1, status="Done"),
            _make_entry(entry_id=2, status="Extracting"),
        ]
        _populate_table(page, entries)
        page.table.selectAll()
        assert not page.re_extract_btn.isEnabled()
...
class TestApplyThemeUpdatesStyles:
    ...
    def test_apply_theme_updates_all_button_styles(self, page) -> None:  # noqa: ANN001
        ...
        assert page.re_extract_btn.styleSheet()
...
class TestExtendedButtonStateCombinations:
    ...
    def test_all_pending_selection(self, page) -> None:  # noqa: ANN001
        """Selecting all Pending entries disables re-extract."""
        entries = [
    ...
    def test_all_failed_selection(self, page) -> None:  # noqa: ANN001
        """Selecting all Failed entries enables re-extract and delete."""
        entries = [
    ...
    def test_all_extracting_selection(self, page) -> None:  # noqa: ANN001
        """Selecting all Extracting entries disables re-extract."""
        entries = [
            _make_entry(entry_id=1, status="Extracting"),
            _make_entry(entry_id=2, status="Extracting"),
        ]
        _populate_table(page, entries)
        page.table.selectAll()
    ...
    def test_mixed_done_and_failed(self, page) -> None:  # noqa: ANN001
        """Mixed Done+Failed enables re-extract and delete."""
        entries = [
            _make_entry(entry_id=1, status="Done"),
            _make_entry(entry_id=2, status="Failed"),
        ]
    ...
    def test_mixed_failed_and_pending(self, page) -> None:  # noqa: ANN001
        """Mixed Failed+Pending disables re-extract."""
        entries = [
            _make_entry(entry_id=1, status="Failed"),
            _make_entry(entry_id=2, status="Pending"),
        ]
    ...
    def test_mixed_done_failed_extracting(self, page) -> None:  # noqa: ANN001
        """Mixed Done+Failed+Extracting disables re-extract."""
        entries = [
            _make_entry(entry_id=1, status="Done"),
            _make_entry(entry_id=2, status="Failed"),
            _make_entry(entry_id=3, status="Extracting"),
        ]
        _populate_table(page, entries)
        page.table.selectAll()
        assert not page.re_extract_btn.isEnabled()
...
# ===================================================================
# NEW: Re-Extract Edge Cases
# ===================================================================


class TestReExtractEdgeCases:
    """Tests for re-extract edge cases."""

    @patch(f"{_MOD}.Path.exists", return_value=True)
    def test_re_extract_single_correct_task(
        self,
        mock_exists,
        page,
        qtbot,  # noqa: ANN001
    ) -> None:
        """Re-extract single file emits correct task tuple."""
        _populate_table(
            page,
            [_make_entry(entry_id=42, source_path="/src/img.png", status="Done")],
        )
        page.table.selectRow(0)
        with qtbot.waitSignal(page.re_extract_requested, timeout=1000) as sig:
            page.on_re_extract()
        assert sig.args[0] == [(42, "/src/img.png")]

    @patch(f"{_MOD}.Path.exists", side_effect=[True, False])
    @patch(f"{_MOD}.CustomMessageDialog.show_message")
    @patch(f"{_MOD}.delete_extraction_entry")
    def test_re_extract_second_file_missing(
        self,
        mock_delete,
        mock_msg,
        mock_exists,
        ...
        """Second file missing stops re-extract and shows error."""
        entries = [
            _make_entry(entry_id=1, source_path="/src/a.png", status="Done"),
            _make_entry(entry_id=2, source_path="/src/gone.png", status="Done"),
        ]
        _populate_table(page, entries)
        page.table.selectAll()
        with (
...
class TestSearchFilteringExtended:
    ...
    def test_search_multiple_matches(self, page) -> None:  # noqa: ANN001
        ...
        entries = [
            _make_entry(entry_id=1, name="scan_page1.png"),
            _make_entry(entry_id=2, name="scan_page2.png"),
            _make_entry(entry_id=3, name="photo.jpg"),
        ]
        page.search_input.setText("scan")
        _populate_table(page, entries)
        assert page.table.rowCount() == 2  # noqa: PLR2004
    ...
    def test_search_by_extension(self, page) -> None:  # noqa: ANN001
        ...
        entries = [
            _make_entry(entry_id=1, name="file.png"),
            _make_entry(entry_id=2, name="file.jpg"),
        ]
        page.search_input.setText(".jpg")
        _populate_table(page, entries)
        assert page.table.rowCount() == 1
    ...
    def test_search_with_dots(self, page) -> None:  # noqa: ANN001
        ...
        entries = [_make_entry(entry_id=1, name="scan.v2.png")]
        page.search_input.setText("v2.png")
        _populate_table(page, entries)
        assert page.table.rowCount() == 1

    def test_search_mixed_case(self, page) -> None:  # noqa: ANN001
        """Search works with mixed case."""
        entries = [_make_entry(entry_id=1, name="MyImage.PNG")]
        page.search_input.setText("myimage")
        _populate_table(page, entries)
        assert page.table.rowCount() == 1
...
class TestThemeAndLanguageExtended:
    ...
    def test_apply_language_preserves_data(self, page) -> None:  # noqa: ANN001
        ...
        entries = [_make_entry(entry_id=99, name="test.png")]
        _populate_table(page, entries)
        with (
            patch(f"{_MOD}.get_extraction_fingerprint", return_value=(1, 1, "x")),
            patch(f"{_MOD}.get_extraction_history", return_value=entries),
        ):
            page.apply_language()
        assert page.table.rowCount() == 1
...
class TestOpenFileExtended:
    ...
    def test_open_multiple_files(
        ...
        entries = [
            _make_entry(entry_id=1, output_path="/out/a.txt"),
            _make_entry(entry_id=2, output_path="/out/b.txt"),
        ]
        _populate_table(page, entries)
        page.table.selectAll()
        page.on_open_file()
...
class TestLargeDataset:
    ...
    def test_ten_entries(self, page) -> None:  # noqa: ANN001
        ...
        entries = [_make_entry(entry_id=i, name=f"img{i}.png") for i in range(1, 11)]
        _populate_table(page, entries)
...
class TestDeleteExtended:
    ...
    @patch(f"{_MOD}.Path.is_file", return_value=True)
    @patch(f"{_MOD}.Path.unlink")
    @patch(f"{_MOD}.delete_extraction_entry", return_value="/out/img.txt")
    @patch(f"{_MOD}.CustomConfirmDialog.confirm", return_value=True)
...
class TestSortingBehaviorExtended:
    ...
    def test_sort_by_size_ascending(self, page) -> None:  # noqa: ANN001
        """Sorting by size column orders rows numerically."""
        entries = [
            _make_entry(entry_id=1, name="big.png", file_size=10000),
            _make_entry(entry_id=2, name="small.png", file_size=100),
        ]
        _populate_table(page, entries)
        page.table.sortByColumn(1, Qt.SortOrder.AscendingOrder)
        first_id = page.table.item(0, 0).data(Qt.ItemDataRole.UserRole)
...
class TestUnicodeAndLongFilenames:
    ...
    def test_unicode_filename(self, page) -> None:  # noqa: ANN001
        """Unicode filenames are displayed correctly."""
        _populate_table(page, [_make_entry(name="\u56fe\u50cf\u6587\u4ef6.png")])
        assert page.table.item(0, 0).text() == "\u56fe\u50cf\u6587\u4ef6.png"
    ...
    def test_unicode_search(self, page) -> None:  # noqa: ANN001
        """Search works with unicode characters."""
        entries = [
            _make_entry(entry_id=1, name="\u56fe\u50cf.png"),
            _make_entry(entry_id=2, name="english.png"),
        ]
        page.search_input.setText("\u56fe\u50cf")
        _populate_table(page, entries)
        assert page.table.rowCount() == 1
...
class TestErrorBannerExtended:
    ...
    def test_error_hidden_for_extracting_status(self, page) -> None:  # noqa: ANN001
        """Error banner hidden for Extracting entry."""
        _populate_table(page, [_make_entry(status="Extracting")])
        page.table.selectRow(0)
        assert not page.error_frame.isVisible()
...
class TestDataStorageVerification:
    ...
    def test_stores_source_path_data(self, page) -> None:  # noqa: ANN001
        ...
        _populate_table(page, [_make_entry(source_path="/src/test.png")])
        item = page.table.item(0, 0)
        assert item.data(Qt.ItemDataRole.UserRole + 2) == "/src/test.png"
    ...
    def test_output_path_stored(self, page) -> None:  # noqa: ANN001
        """Output path is stored in UserRole+1 on column 0."""
        _populate_table(page, [_make_entry(output_path="/out/extract.txt")])
        item = page.table.item(0, 0)
        assert item.data(Qt.ItemDataRole.UserRole + 1) == "/out/extract.txt"
...
# ===================================================================
# NEW: Re-Extract Multiple Extended
# ===================================================================


class TestReExtractMultipleExtended:
    """Extended tests for re-extracting multiple files."""

    @patch(f"{_MOD}.Path.exists", return_value=True)
    def test_re_extract_three_files(
        self,
        mock_exists,
        page,
        qtbot,  # noqa: ANN001
    ) -> None:
        """Re-extract with three selected emits three tasks."""
        entries = [
            _make_entry(entry_id=i, source_path=f"/src/i{i}.png", status="Done")
            for i in range(1, 4)
        ]
        _populate_table(page, entries)
        page.table.selectAll()
    ...
    @patch(f"{_MOD}.Path.exists", return_value=True)
    def test_re_extract_failed_entries(
        self,
        mock_exists,
        page,
        qtbot,  # noqa: ANN001
    ) -> None:
        """Re-extract works with failed entries."""
        entries = [
            _make_entry(entry_id=1, source_path="/src/i1.png", status="Failed"),
            _make_entry(entry_id=2, source_path="/src/i2.png", status="Failed"),
        ]
        _populate_table(page, entries)
        page.table.selectAll()
        with qtbot.waitSignal(page.re_extract_requested, timeout=1000) as sig:
...
class TestTableStructureVerification:
    ...
    def test_file_name_displayed_correctly(self, page) -> None:  # noqa: ANN001
        """File name is shown as column 0 text."""
        _populate_table(page, [_make_entry(name="scan_doc.png")])
        assert page.table.item(0, 0).text() == "scan_doc.png"
    ...
    def test_search_partial_match(self, page) -> None:  # noqa: ANN001
        """Search matches partial file names."""
        entries = [_make_entry(entry_id=1, name="document_scan_page1.png")]
        page.search_input.setText("scan")
        _populate_table(page, entries)
        assert page.table.rowCount() == 1
    ...
    def test_refresh_empty_history_struct(self, page) -> None:  # noqa: ANN001
        """Empty history list results in 0 rows."""
        _populate_table(page, [])
        assert page.table.rowCount() == 0

    def test_refresh_clears_on_none_struct(self, page) -> None:  # noqa: ANN001
        """Table is cleared when get_extraction_history returns None."""
        with (
            patch(f"{_MOD}.get_extraction_fingerprint", return_value=(0, 0, "x")),
            patch(f"{_MOD}.get_extraction_history", return_value=None),
...
# ===================================================================
# NEW: Extraction Extended Coverage
# ===================================================================


class TestExtractionExtendedCoverage:
    """Extended coverage tests for extraction history page."""

    def test_error_banner_hidden_on_multi_select_ext(self, page) -> None:  # noqa: ANN001
    ...
    def test_delete_btn_enabled_for_failed_ext(self, page) -> None:  # noqa: ANN001
        """Delete button is enabled for Failed status entries."""
        _populate_table(page, [_make_entry(status="Failed")])
        page.table.selectRow(0)
        assert page.delete_btn.isEnabled()

    def test_re_extract_btn_disabled_for_extracting_ext(self, page) -> None:  # noqa: ANN001
        """Re-extract is disabled when an Extracting row is selected."""
        _populate_table(page, [_make_entry(status="Extracting")])
        page.table.selectRow(0)
        assert not page.re_extract_btn.isEnabled()

    def test_re_extract_btn_disabled_for_pending_ext(self, page) -> None:  # noqa: ANN001
        """Re-extract is disabled when a Pending row is selected."""
        _populate_table(page, [_make_entry(status="Pending")])
        page.table.selectRow(0)
        assert not page.re_extract_btn.isEnabled()

    def test_open_btn_enabled_for_done_ext(self, page) -> None:  # noqa: ANN001
        """Open button is enabled when a Done row is selected."""
        _populate_table(page, [_make_entry(status="Done")])
        page.table.selectRow(0)
        assert page.open_btn.isEnabled()

    def test_search_timer_is_single_shot_ext(self, page) -> None:  # noqa: ANN001
        """Search timer is configured as single-shot."""
        assert page.search_timer.isSingleShot()

    def test_background_timer_not_single_shot_ext(self, page) -> None:  # noqa: ANN001
        """Background refresh timer is not single-shot (repeating)."""
        assert not page.timer.isSingleShot()

    def test_table_selection_mode_extended_ext(self, page) -> None:  # noqa: ANN001
    ...
    def test_table_no_edit_triggers_ext(self, page) -> None:  # noqa: ANN001
        """Table has no edit triggers (read-only)."""
        from PySide6.QtWidgets import QAbstractItemView  # noqa: PLC0415

        assert page.table.editTriggers() == QAbstractItemView.EditTrigger.NoEditTriggers

    def test_error_banner_visible_single_failed_ext(self, page) -> None:  # noqa: ANN001
    ...
    def test_error_banner_hidden_for_done_ext(self, page) -> None:  # noqa: ANN001
        """Error banner is hidden when a Done entry is selected."""
    ...
    def test_highlight_delegate_exists_ext(self, page) -> None:  # noqa: ANN001
        """Page has a highlight delegate for file name column."""
        assert page.highlight_delegate is not None

    def test_status_delegate_exists_ext(self, page) -> None:  # noqa: ANN001
        """Page has a status delegate for status column."""
        assert page._status_delegate is not None

    def test_buttons_disabled_initially_ext(self, page) -> None:  # noqa: ANN001
    ...
    def test_main_layout_zero_margins_ext(self, page) -> None:  # noqa: ANN001
        """Main layout has zero margins."""
        margins = page.main_layout.contentsMargins()
        assert margins.left() == 0
        assert margins.top() == 0
    ...
    def test_re_extract_enabled_for_done_ext(self, page) -> None:  # noqa: ANN001
        """Re-extract is enabled for a Done row."""
        _populate_table(page, [_make_entry(status="Done")])
        page.table.selectRow(0)
        assert page.re_extract_btn.isEnabled()

    def test_re_extract_enabled_for_failed_ext(self, page) -> None:  # noqa: ANN001
        """Re-extract is enabled for a Failed row."""
        _populate_table(page, [_make_entry(status="Failed")])
        page.table.selectRow(0)
        assert page.re_extract_btn.isEnabled()

    def test_error_frame_starts_hidden_ext(self, page) -> None:  # noqa: ANN001
        """Error frame starts hidden (no selection)."""
        page.table.clearSelection()
        page._update_button_states()
        assert not page.error_frame.isVisible()

    def test_table_four_columns_ext(self, page) -> None:  # noqa: ANN001
        """Table has exactly 4 columns (name, size, status, date)."""
        assert page.table.columnCount() == 4  # noqa: PLR2004

    def test_search_input_max_width_ext(self, page) -> None:  # noqa: ANN001
        """Search input has a maximum width set."""
        assert page.search_input.maximumWidth() == 360  # noqa: PLR2004

    def test_page_attribute_exists_ext(self, page) -> None:  # noqa: ANN001
        """Page has a page container attribute."""
        assert hasattr(page, "page")

    def test_actions_layout_exists_ext(self, page) -> None:  # noqa: ANN001
        """Page has an actions layout attribute."""
        assert hasattr(page, "actions_layout")


# ===================================================================
# TestExtractionHistorySignalBlocking — blockSignals & setSortingEnabled
# ===================================================================


class TestExtractionHistorySignalBlocking:
    ...
    def test_block_signals_called_in_correct_order(self, page) -> None:  # noqa: ANN001
        ...
        # True must appear before the last False
