"""Comprehensive tests for SubtitleHistoryPage.
...
# ---------------------------------------------------------------------------
_MOD = "src.ui.pages.subtitle_history"
...
def _mock_db():
    ...
    with (
        patch(f"{_MOD}.get_subtitle_fingerprint", return_value=None),
        patch(f"{_MOD}.get_subtitle_history", return_value=[]),
    ):
        yield
...
def _make_entry(  # noqa: PLR0913
    ...
    name: str = "video.mp4",
    file_size: int = 2048,
    source_path: str = "/tmp/source/video.mp4",
    output_path: str = "/tmp/output/video.srt",
    src_lang: str = "en",
    status: str = "Done",
    error_message: str | None = None,
    created_at: str = "2026-02-10 14:00:00",
) -> tuple:
    """Helper to build a subtitle history DB tuple."""
    return (
        entry_id,
        name,
        file_size,
...
def _populate_table(page, entries):  # noqa: ANN001, ANN202
    ...
    with (
        patch(
            f"{_MOD}.get_subtitle_fingerprint",
            return_value=(len(entries), len(entries), "x"),
        ),
        patch(f"{_MOD}.get_subtitle_history", return_value=entries),
    ):
...
class TestConstruction:
    """Tests for SubtitleHistoryPage widget construction."""
...
class TestRefreshHistory:
    ...
    def test_refresh_clears_on_none(self, page) -> None:  # noqa: ANN001
        """Table is cleared when get_subtitle_history returns None."""
        with (
            patch(f"{_MOD}.get_subtitle_fingerprint", return_value=(0, 0, "x")),
            patch(f"{_MOD}.get_subtitle_history", return_value=None),
        ):
            page.refresh_history(force=True)
    ...
    def test_refresh_skips_when_fingerprint_unchanged(self, page) -> None:  # noqa: ANN001
        ...
        with (
            patch(f"{_MOD}.get_subtitle_fingerprint", return_value=fp),
            patch(f"{_MOD}.get_subtitle_history", return_value=[]),
        ):
            page.show()
        with (
            patch(f"{_MOD}.get_subtitle_fingerprint", return_value=fp),
            patch(f"{_MOD}.get_subtitle_history", history_mock),
        ):
...
class TestFillRow:
    ...
    def test_stores_output_path(self, page) -> None:  # noqa: ANN001
        ...
        _populate_table(page, [_make_entry(output_path="/out/sub.srt")])
        item = page.table.item(0, 0)
        assert item.data(Qt.ItemDataRole.UserRole + 1) == "/out/sub.srt"

    def test_stores_source_path(self, page) -> None:  # noqa: ANN001
        """Source path is stored in UserRole+2 on column 0."""
        _populate_table(page, [_make_entry(source_path="/src/vid.mp4")])
        item = page.table.item(0, 0)
        assert item.data(Qt.ItemDataRole.UserRole + 2) == "/src/vid.mp4"

    def test_stores_error_message(self, page) -> None:  # noqa: ANN001
        """Error message is stored in UserRole+3 on column 0."""
        _populate_table(page, [_make_entry(error_message="TIMEOUT_ERROR")])
        item = page.table.item(0, 0)
        assert item.data(Qt.ItemDataRole.UserRole + 3) == "TIMEOUT_ERROR"

    def test_file_name_displayed(self, page) -> None:  # noqa: ANN001
        """File name is shown as column 0 text."""
        _populate_table(page, [_make_entry(name="interview.mp4")])
        assert page.table.item(0, 0).text() == "interview.mp4"

    def test_size_formatted(self, page) -> None:  # noqa: ANN001
        """File size is formatted via format_file_size."""
        _populate_table(page, [_make_entry(file_size=5000)])
...
class TestButtonStates:
    ...
    def test_error_message_shown_for_failed(self, page) -> None:  # noqa: ANN001
        ...
        page.show()
        _populate_table(page, [_make_entry(status="Failed", error_message="ERR_STT")])
        page.table.selectRow(0)
    ...
    def test_auth_error_service_suffix_renders_specific_service(
        ...
        Pins the end-to-end display chain: persisted engine tag
        (with ``:Service`` suffix) → ``display_error_message``
        suffix parsing → localised text containing the service
        name.  Regression guard for the bug where the suffix would
        be stripped (or the exact-match check missed) and the
        generic fallback message would surface, hiding which key
        the user must fix.
        """
...
class TestOpenFile:
    ...
    def test_open_file_skips_missing(
        ...
        _populate_table(page, [_make_entry(output_path="/out/missing.srt")])
        page.table.selectRow(0)
        page.on_open_file()
...
class TestDeleteSelected:
    ...
    @patch(f"{_MOD}.delete_subtitle_entry", return_value="/out/sub.srt")
...
class TestReGenerate:
    ...
    def test_re_generate_emits_signal(
        ...
        _populate_table(
            page,
            [_make_entry(entry_id=7, source_path="/src/vid.mp4", status="Done")],
        )
        page.table.selectRow(0)
        ...
        assert sig.args[0][0] == (7, "/src/vid.mp4")
...
class TestSearchFiltering:
    ...
    def test_search_filters_by_name(self, page) -> None:  # noqa: ANN001
        ...
        entries = [
            _make_entry(entry_id=1, name="Interview.mp4"),
            _make_entry(entry_id=2, name="Lecture.mp4"),
        ]
        page.search_input.setText("interview")
        _populate_table(page, entries)
...
class TestPageIsQWidget:
    ...
    def test_is_qwidget_instance(self, page) -> None:  # noqa: ANN001
        """SubtitleHistoryPage is a QWidget."""
...
class TestTableRefreshPreservation:
    ...
    def test_refresh_not_visible_no_force_skips(self, page) -> None:  # noqa: ANN001
        ...
        with (
            patch(f"{_MOD}.get_subtitle_fingerprint", return_value=(1, 1, "new")),
            patch(f"{_MOD}.get_subtitle_history", history_mock),
        ):
            page.refresh_history(force=False)
            history_mock.assert_not_called()
...
class TestUnicodeAndLongFilenames:
    ...
    def test_unicode_filename(self, page) -> None:  # noqa: ANN001
        ...
        _populate_table(page, [_make_entry(name="\u5b57\u5e55\u6587\u4ef6.mp4")])
        assert page.table.item(0, 0).text() == "\u5b57\u5e55\u6587\u4ef6.mp4"

    def test_emoji_filename(self, page) -> None:  # noqa: ANN001
        """Emoji in filenames are displayed correctly."""
        _populate_table(page, [_make_entry(name="\U0001f3a4 podcast.mp4")])
        assert page.table.item(0, 0).text() == "\U0001f3a4 podcast.mp4"
    ...
    def test_unicode_search(self, page) -> None:  # noqa: ANN001
        ...
        entries = [
            _make_entry(entry_id=1, name="\u5b57\u5e55.mp4"),
            _make_entry(entry_id=2, name="english.mp4"),
        ]
        page.search_input.setText("\u5b57\u5e55")
        _populate_table(page, entries)
        assert page.table.rowCount() == 1
...
class TestDeleteFileCleanup:
    ...
    @patch(f"{_MOD}.delete_subtitle_entry", return_value=None)
    @patch(f"{_MOD}.CustomConfirmDialog.confirm", return_value=True)
...
class TestSignals:
    ...
    def test_re_generate_requested_signal_exists(self, page) -> None:  # noqa: ANN001
        """SubtitleHistoryPage has re_generate_requested signal."""
...
class TestReGenerateEdgeCases:
    ...
    def test_re_generate_single_correct_task(
        ...
        _populate_table(
            page,
            [_make_entry(entry_id=42, source_path="/src/v.mp4", status="Done")],
        )
        page.table.selectRow(0)
        with qtbot.waitSignal(page.re_generate_requested, timeout=1000) as sig:
            page.on_re_generate()
        assert sig.args[0] == [(42, "/src/v.mp4")]
    ...
    def test_re_generate_second_file_missing(
        ...
        entries = [
            _make_entry(entry_id=1, source_path="/src/a.mp4", status="Done"),
            _make_entry(entry_id=2, source_path="/src/gone.mp4", status="Done"),
        ]
        _populate_table(page, entries)
        page.table.selectAll()
        with (
...
class TestSearchFilteringExtended:
    ...
    def test_search_multiple_matches(self, page) -> None:  # noqa: ANN001
        ...
        entries = [
            _make_entry(entry_id=1, name="video_part1.mp4"),
            _make_entry(entry_id=2, name="video_part2.mp4"),
            _make_entry(entry_id=3, name="audio.mp3"),
        ]
        page.search_input.setText("video")
        _populate_table(page, entries)
        assert page.table.rowCount() == 2  # noqa: PLR2004
    ...
    def test_search_by_extension(self, page) -> None:  # noqa: ANN001
        ...
        entries = [
            _make_entry(entry_id=1, name="file.mp4"),
            _make_entry(entry_id=2, name="file.mkv"),
        ]
        page.search_input.setText(".mkv")
        _populate_table(page, entries)
        assert page.table.rowCount() == 1
    ...
    def test_search_with_dots(self, page) -> None:  # noqa: ANN001
        ...
        entries = [_make_entry(entry_id=1, name="file.v2.mp4")]
        page.search_input.setText("v2.mp4")
        _populate_table(page, entries)
        assert page.table.rowCount() == 1

    def test_search_mixed_case(self, page) -> None:  # noqa: ANN001
        """Search works with mixed case."""
        entries = [_make_entry(entry_id=1, name="MyVideo.MP4")]
        page.search_input.setText("myvideo")
        _populate_table(page, entries)
        assert page.table.rowCount() == 1
...
class TestThemeAndLanguageExtended:
    ...
    def test_apply_language_preserves_data(self, page) -> None:  # noqa: ANN001
        ...
        entries = [_make_entry(entry_id=99, name="test.mp4")]
        _populate_table(page, entries)
        with (
            patch(f"{_MOD}.get_subtitle_fingerprint", return_value=(1, 1, "x")),
            patch(f"{_MOD}.get_subtitle_history", return_value=entries),
        ):
            page.apply_language()
        assert page.table.rowCount() == 1
...
class TestOpenFileExtended:
    ...
    def test_open_multiple_files(
        ...
        entries = [
            _make_entry(entry_id=1, output_path="/out/a.srt"),
            _make_entry(entry_id=2, output_path="/out/b.srt"),
        ]
        _populate_table(page, entries)
        page.table.selectAll()
        page.on_open_file()
...
class TestLargeDataset:
    ...
    def test_ten_entries(self, page) -> None:  # noqa: ANN001
        ...
        entries = [_make_entry(entry_id=i, name=f"file{i}.mp4") for i in range(1, 11)]
        _populate_table(page, entries)
...
class TestSortingBehaviorExtended:
    ...
    def test_sort_by_size_ascending(self, page) -> None:  # noqa: ANN001
        ...
        entries = [
            _make_entry(entry_id=1, name="big.mp4", file_size=10000),
            _make_entry(entry_id=2, name="small.mp4", file_size=100),
        ]
        _populate_table(page, entries)
...
class TestErrorBannerExtended:
    ...
    def test_error_hidden_when_no_error_message(self, page) -> None:  # noqa: ANN001
        """Error banner hidden for failed entry without error_message."""
        _populate_table(page, [_make_entry(status="Failed", error_message=None)])
        page.table.selectRow(0)
        assert not page.error_frame.isVisible()
    ...
    def test_error_hidden_after_deselection(self, page) -> None:  # noqa: ANN001
        """Error banner hidden after deselecting failed entry."""
        page.show()
        _populate_table(
            page,
            [_make_entry(status="Failed", error_message="ERR")],
        )
        page.table.selectRow(0)
        assert page.error_frame.isVisible()
        page.table.clearSelection()
...
# ===================================================================
# NEW: Page Is QWidget Extended
# ===================================================================


class TestPageIsQWidgetExtended:
    """Tests that the page is a proper QWidget."""
...
class TestUnicodeAndLongFilenamesExtended:
    """Tests for edge cases with unicode and very long filenames."""
    ...
    def test_emoji_filename(self, page) -> None:  # noqa: ANN001
        """Emoji in filenames are displayed correctly."""
        _populate_table(page, [_make_entry(name="\U0001f3a4 speech.mp4")])
        assert page.table.item(0, 0).text() == "\U0001f3a4 speech.mp4"
...
# ===================================================================
# NEW: Refresh Visibility Extended
# ===================================================================


class TestRefreshVisibilityExtended:
...
class TestDataStorageVerification:
    ...
    def test_stores_source_path_data(self, page) -> None:  # noqa: ANN001
        ...
        _populate_table(page, [_make_entry(source_path="/src/test.mp4")])
        item = page.table.item(0, 0)
        assert item.data(Qt.ItemDataRole.UserRole + 2) == "/src/test.mp4"

    def test_stores_error_message_data(self, page) -> None:  # noqa: ANN001
        """Error message is stored in UserRole+3 on column 0."""
        _populate_table(page, [_make_entry(error_message="ERR_STT")])
        item = page.table.item(0, 0)
        assert item.data(Qt.ItemDataRole.UserRole + 3) == "ERR_STT"
    ...
    def test_size_column_for_large_file(self, page) -> None:  # noqa: ANN001
        ...
        size_text = page.table.item(0, 1).text()
        assert size_text  # Should show formatted size like "1.0 MB"
...
class TestReGenerateMultipleExtended:
    ...
    def test_re_generate_three_files(
        ...
        entries = [
            _make_entry(entry_id=i, source_path=f"/src/v{i}.mp4", status="Done")
            for i in range(1, 4)
        ]
        _populate_table(page, entries)
        page.table.selectAll()
    ...
    def test_re_generate_failed_entries(
        ...
        entries = [
            _make_entry(entry_id=1, source_path="/src/v1.mp4", status="Failed"),
            _make_entry(entry_id=2, source_path="/src/v2.mp4", status="Failed"),
        ]
        _populate_table(page, entries)
        page.table.selectAll()
        with qtbot.waitSignal(page.re_generate_requested, timeout=1000) as sig:
...
class TestPageStructureExtended:
    ...
    def test_error_frame_hidden_initially(self, page) -> None:  # noqa: ANN001
        """Error banner is hidden at construction time (separate check)."""
        assert not page.error_frame.isVisible()
    ...
    def test_search_uppercase_query(self, page) -> None:  # noqa: ANN001
        """Search is case-insensitive with uppercase query."""
        entries = [_make_entry(entry_id=1, name="video.mp4")]
        page.search_input.setText("VIDEO")
        _populate_table(page, entries)
    ...
    def test_search_updates_highlight(self, page) -> None:  # noqa: ANN001
        """Search text is passed to the highlight delegate."""
        entries = [_make_entry(entry_id=1, name="test.mp4")]
        page.search_input.setText("test")
        _populate_table(page, entries)
...
class TestAdditionalEdgeCases:
    """Additional edge-case tests for subtitle history page."""

    def test_error_banner_hidden_on_multi_select(self, page) -> None:  # noqa: ANN001
        """Error banner stays hidden when multiple rows are selected."""
        entries = [
    ...
    def test_delete_btn_enabled_for_failed_subtitle(self, page) -> None:  # noqa: ANN001
        """Delete button is enabled for Failed status entries."""
        _populate_table(page, [_make_entry(status="Failed")])
        page.table.selectRow(0)
        assert page.delete_btn.isEnabled()

    def test_re_generate_btn_disabled_for_generating_subtitle(self, page) -> None:  # noqa: ANN001
        """Re-generate is disabled when a Generating row is selected."""
        _populate_table(page, [_make_entry(status="Generating")])
        page.table.selectRow(0)
        assert not page.re_generate_btn.isEnabled()

    def test_re_generate_btn_disabled_for_pending_subtitle(self, page) -> None:  # noqa: ANN001
        """Re-generate is disabled when a Pending row is selected."""
        _populate_table(page, [_make_entry(status="Pending")])
        page.table.selectRow(0)
        assert not page.re_generate_btn.isEnabled()

    def test_open_btn_enabled_for_done_subtitle(self, page) -> None:  # noqa: ANN001
        """Open button is enabled when a Done row is selected."""
        _populate_table(page, [_make_entry(status="Done")])
        page.table.selectRow(0)
        assert page.open_btn.isEnabled()

    def test_search_timer_is_single_shot_subtitle(self, page) -> None:  # noqa: ANN001
        """Search timer is configured as single-shot."""
        assert page.search_timer.isSingleShot()

    def test_background_timer_not_single_shot_subtitle(self, page) -> None:  # noqa: ANN001
        """Background refresh timer is not single-shot (repeating)."""
        assert not page.timer.isSingleShot()

    def test_table_selection_mode_extended_subtitle(self, page) -> None:  # noqa: ANN001
...
# ===================================================================
# TestSubtitleHistorySignalBlocking — blockSignals & setSortingEnabled
# ===================================================================


class TestSubtitleHistorySignalBlocking:
