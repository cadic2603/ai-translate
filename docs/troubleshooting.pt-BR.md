---
description: Diagnostique problemas do AI Translate — fontes faltando, erros de FFmpeg, problemas do conversor LibreOffice, falhas de configuração OCR e autenticação da API LLM.
---
...
**Configurações → LLM** → cole uma chave. Veja
...
- Provedores personalizados — certifique-se que o modelo está na lista
  **Models** separada por vírgulas em **Configurações → LLM**.
- Lista Gemini integrada — troque para um modelo documentado em
  **Configurações → LLM → Modelo Gemini padrão**.
...
FFmpeg não está no PATH. Veja [Configuração FFmpeg](setup/ffmpeg.md)
...
- **Fonte de áudio errada** — abra **Configurações → Live → Fonte de
  áudio** e certifique-se que corresponde ao que você realmente quer
  (Microfone / Sistema / Ambos).
- **Mic mutado** — verifique configurações de som do OS; o app usa
...
- **ElevenLabs sem chave** — a página Voice mostra um diálogo amigável
  pre-flight; se passou, o arquivo pode ser bytes vazios. Verifique
  **Configurações → Service → ElevenLabs API key**.
...
1. Clique em **Abrir Configurações** no diálogo (ou abra
   **Configurações → Voz** manualmente).
2. Certifique-se que **Método TTS** está em **Piper TTS**.
3. Clique em **Baixar vozes agora** para abrir o diálogo da biblioteca
   de vozes.
4. Encontre a linha do seu idioma alvo, depois clique em **Female
   voice** ou **Male voice** ao lado. As vozes são arquivos ONNX de
   ~25–60 MB baixados do HuggingFace; uma vez por idioma.
...
você não precisa baixar nada de fato. Os 13 idiomas sem vozes Piper
...
- A página é uma **digitalização sem camada de texto** e OCR não está
  configurado. Configure **Configurações → OCR** (veja
...
- **Formatos legacy** (`.doc`, `.xls`, `.ppt`) — ative
  **Configurações → Tradução → Auto-convert legacy** para fidelidade
...
### Múltiplos arquivos de uma vez traduzidos silenciosamente como um

Solte um arquivo de cada vez, OU verifique a coluna **Files** no
cabeçalho da fila — deve mostrar a contagem que você soltou. O cap de
100 arquivos mostra uma notificação quando excedido.
...
para espelhar o log de arquivo completo em stdout.
