...
1. Cole em **Chave de API Google Cloud** → **Guardar**
...
Em **Definições → OCR → Método OCR = Google Cloud OCR**.

É isso — vai usar a mesma chave do Serviço.

### Speech-to-Text

Em **Definições → Legenda → Método STT = Google Cloud** (para as
páginas Legenda / Voz) ou **Definições → Live → Método STT =
Google Cloud** (para a página Live).

Em **Definições → Legenda → Modelo STT Google**, escolha o
...
| Erro | Causa provável |
|---|---|
| `AUTH_ERROR` | Chave errada / expirada. Cole novamente em Definições → Serviço. |
