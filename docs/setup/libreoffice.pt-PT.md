---
description: Instale o LibreOffice para que o AI Translate possa traduzir formatos Office legados (.doc, .xls, .ppt) e ficheiros ODF (.odt, .ods, .odp) no macOS e Linux.
---
...
=== "macOS"
    ...
    Ou descarregue de <https://www.libreoffice.org/download/download/>.
...
pode sobrescrever em **Definições → Geral → Caminho do
...
!!! info "Logs de crash JVM"
    O componente Java do LibreOffice ocasionalmente produz ficheiros
...
!!! tip "Auto-converter legado / ODF"
    Ative **Definições → Tradução → Auto-converter legado** se
