...
1. Cole a chave em **Chave de API ElevenLabs** → **Guardar**
...
|---|---|
| **Gerar voz** | Você quer narrações de qualidade premium a partir de ficheiros de legenda |
...
ID de voz em **Definições → Serviço → ID da voz** e o pipeline
...
!!! warning "Verificação pre-flight"
    ...
    você receberá um diálogo amigável apontando para as Definições,
...
!!! info "FFmpeg ainda necessário"
    ...
    ficheiro. Veja [Definição do FFmpeg](ffmpeg.md).
...
| `MODEL_NOT_FOUND` | O modelo ElevenLabs selecionado não está mais disponível; escolha outro em Definições → Voz. |
