...
- **Piper TTS** quando você precisa de saída de voz **offline /
  ...
  voz de ~25–60 MB via **Configurações → Voz → Piper TTS → Baixar
...
Solte a pasta na drop zone de **Traduzir documento**. Arquivos
suportados dentro (recursivamente) entram na fila; tudo o mais é
silenciosamente ignorado. Há um cap de 100 arquivos por drop; lotes
...
edição salva automaticamente no registro de histórico da entrada.

Para **Traduzir documento** — abra o arquivo traduzido no seu editor
...
é togglable em **Configurações → Geral**.
...
headless) é um INI em texto plano sob o diretório de configuração do
seu usuário; nesse modo as chaves são protegidas por permissões de
arquivo mas não criptografadas.
