...
Pipeline completo **STT → Traduzir → TTS → Mix** que produz um ficheiro
...
- **FFmpeg** no `PATH` — veja [Definição FFmpeg](../setup/ffmpeg.md).
- Um backend STT (padrão: Whisper local, sem definição)
- Um backend TTS — Edge TTS (padrão), ElevenLabs, Google Cloud TTS,
  Gemini TTS, ou **Piper TTS** (totalmente offline; vozes por idioma
  baixadas uma vez via **Definições → Voz → Piper TTS → Descarregar
  vozes agora**)
...
2. Solte um ou mais ficheiros de vídeo (`.mp4`, `.webm`, `.mkv`,
   `.avi`, `.mov`, `.wmv`).
...
Para cada vídeo de entrada você obtém quatro ficheiros no diretório
de saída:
...
Clique com o botão direito em uma entrada Done / Failed para estas opções:
...
!!! tip "Verificações pre-flight"
    ...
    botão **Abrir Definições** que te leva à biblioteca de vozes,
