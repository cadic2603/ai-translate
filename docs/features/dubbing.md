...
| **TTS** | 50–90% | Synthesizing target-language voice for each line |
...
!!! tip "Pre-flight checks"
    ...
    Missing key → friendly dialog, no half-run dub. With **Piper TTS**
    selected, it also checks that the per-language Piper voice is on
    disk; missing voice → modal dialog with an **Open Settings** button
