...
Synthesise subtitle files (with timing) or arbitrary text into MP3 / WAV
...
- **Voice library dialogue** — open via **Settings → Voice → Piper TTS
...
- **Pre-flight gate** — before a Voice run starts, the page checks
  ...
  a modal dialogue with an **Open Settings** button that takes you
...
!!! tip "Pre-flight checks"
    ...
    availability before starting. You'll see a friendly dialogue if anything
    is missing.
