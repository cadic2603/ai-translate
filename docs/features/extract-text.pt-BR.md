---
description: Extraia texto de imagens e capturas de tela usando motores OCR (Tesseract, EasyOCR, Google Vision) ou visão LLM — saída para TXT ou DOCX.
---

# Extrair texto

Tire o texto de imagens — recibos, capturas de tela, documentos
...
Escolha o padrão em **Configurações → Extrair texto → Método de extração**.
...
Configure em **Configurações → OCR**.

## Passo a passo

1. Clique em **Extrair texto** na barra lateral.
2. Solte um ou mais arquivos de imagem (`.png`, `.jpg`, `.jpeg`,
   `.bmp`, `.webp`, `.tiff`, `.tif`).
...
!!! tip "Diálogo Configuração necessária"
    Se você escolher OCR mas nenhum motor OCR estiver configurado
    (ou LLM mas nenhuma chave LLM estiver configurada), a página
    mostra um único diálogo "Configuração necessária" que liga
    diretamente à aba Configurações relevante.
