...
the live flow on a download dialogue would be worse than the fallback.
...
Click **Save Transcript** to export the session to a `.txt` file with
timestamps, speakers, original lines, and translated lines.
