...
chat e teste da sua configuração LLM.
...
- **Reuso de histórico** — cada tradução é salva. Clique em uma
...
- **Reprodução TTS** — clique em **Ouvir** ao lado de qualquer painel
  ...
  **Configurações → Voz → Método TTS** — Edge TTS (padrão),
  ...
  mostra um diálogo modal com botão **Abrir Configurações** para
...
!!! tip "Idiomas RTL"
    ...
    mesmo manejo RTL é levado para a saída de arquivo em todos os
