...
Sintetize ficheiros de legenda (com timing) ou texto arbitrário em
...
- Um backend TTS, um destes:
    ...
    - **ElevenLabs** — pago, qualidade máxima. Veja [Definição do ElevenLabs](../setup/elevenlabs.md).
    - **Google Cloud TTS** — pago, muito bom. Veja [Definição do Google Cloud](../setup/google-cloud.md).
    - **Gemini TTS** — camada gratuita, vozes pré-construídas
      naturais. Reutiliza sua chave API do Gemini existente da aba
      LLM — sem definição extra.
    - **Piper TTS** — TTS neural totalmente offline. Sem chave de API,
      sem chamadas de rede — as vozes são ficheiros ONNX de
      ~25–60 MB descarregados uma vez via **Definições → Voz → Piper
      TTS → Descarregar vozes agora**. 32 dos 45 idiomas do app têm uma
...
2. Solte um ou mais ficheiros de legenda `.srt` / `.vtt` /
   `.ass` / `.ssa`.
3. Escolha o **Idioma** (auto-detectado a partir do nome do ficheiro
...
Você obtém um único ficheiro de áudio com as faixas de voz
...
- **Portão pre-flight** — antes de uma execução de Voz começar, a
  ...
  Definições** que te leva direto para a biblioteca de vozes
...
- **Formato de áudio** — Gemini emite PCM bruto a 24 kHz mono
  ...
  escolheu) para que o ficheiro final corresponda ao seu formato de
  saída selecionado.
...
!!! tip "Re-gerar"
    Clique com o botão direito em uma linha → **Re-gerar** para trocar gênero da
...
!!! warning "Stop é atômico"
    Pressione **Stop** durante a síntese e você não ficará com um MP3
    semi-escrito no diretório de saída — o ficheiro é escrito em uma
