...
| `2` | Erro de argumento (idioma desconhecido, alvo faltando, modelo malformado, saída não gravável, extensão de arquivo não suportada) |
...
atual é salvo, o pipeline sai limpo no próximo limite de polling.

`Ctrl+C` novamente — interrupção dura. Use com moderação; arquivos
...
!!! tip "Configure uma vez, use em todo lugar"
    O CLI compartilha suas chaves de API e configurações com o app
...
!!! tip "Streams de saída"
    ...
    com erros); mensagens de erro e registros de log vão para
