---
description: Instale AI Translate no Windows, macOS ou Linux a partir de binários pré-compilados ou do código-fonte — cobre Python, FFmpeg e configuração opcional do LibreOffice.
---
...
| **LibreOffice** ([download](https://www.libreoffice.org/download/)) | Formatos Office no Linux/macOS | Traduzir legacy `.doc` / `.xls` / `.ppt`, ou qualquer arquivo Office quando MS Office não estiver instalado |
...
3. Cole sua **chave de API Google Gemini** (ou configure um provedor
   personalizado compatível com OpenAI). Usuários enterprise podem
...
5. Feche Configurações — pronto

As chaves são salvas no **chaveiro do seu OS** (Keychain macOS,
